...
def t_error(t):
    if t:
        ...
        print("Error de sintaxis en la entrada: token '{0}', línea {1}, posición {2}".format(t.value, line, column))
    t.lexer.skip(1)
...
def p_error(p):
    ...
    if p:
        ...
        print("Error de sintaxis en la entrada: token '{0}', línea {1}, posición {2}".format(p.value, line, column))
    else:
...
def generar_archivo_html():
    if valido:
        direccion_archivo = input("Ingresa la dirección donde deseas guardar el archivo: ")
        ...
        try:
            ...
            print("Se ha creado el archivo con éxito en la siguiente ruta:", ruta_completa)
