...
tokens = [ "TEXT" ,"NUMERO",                         
        ...
        "IMAGE1", "WIDTH1","WIDTH2",
        "HEIGHT1","HEIGHT2", 
        "RSS2","RSS1", "XML","TOKENURL",
        "VERSION1","VERSION2",
        ]
H=0
a=0
...
def p_error(p):
    global correcto
    correcto=1
    pantalla1.insert('1.0', "Error de sintaxis en la línea "+str(p.lineno)+"\nVerifique la cadena "+str(p.value)+"\n\n")
...
def abrirarchivo():
    global NombreArch
    NombreArch=filedialog.askopenfilename(initialdir = "/./prueba", title = "Select a File", filetypes = (("Rss files", "*.rss*"), ("Text files", "*.txt*"), ("all files", "*.*")))
...
Label_text.config(font=('Helvetica 25 bold'), background="thistle4", foreground="gray10")
Label_text.grid(row=0, column=0, columnspan=5, pady=20)

#Caja de texto
pantalla= Entry(ventana)
pantalla= Text(state="normal", width=100, height=20,background="gray90", foreground="black", font=("Helvetica", 10))
#Ubicar caja
pantalla.grid (row=1, column=0, columnspan=4,padx=10,pady=10)

#Caja de texto1
pantalla1= Entry(ventana)
pantalla1= Text(state="normal", width=100, height=10,background="gray90", foreground="black", font=("Helvetica", 10))
#Ubicar caja1
pantalla1.grid (row=2, column=0, columnspan=4,padx=10,pady=10)

#Botones
style=ttk.Style()
style.configure('W.TButton', font =('calibri', 10, 'bold', 'underline'),border=5, borderwidth=5, background='#ab28bd', bd=5)
#Boton Abrir archivo
ventana.boton_abrirarchivo= ttk.Button(text="Abrir Archivo", command=abrirarchivo, style='W.TButton',width=20).grid(row=3,column=0,pady=15,columnspan=2)
#Boton compilar
ventana.boton_compilar= ttk.Button(text="Compilar", command=copiar, style='W.TButton', width=20).grid(row=3, column=1,padx=10,pady=15,columnspan=2)
#Boton Exit
